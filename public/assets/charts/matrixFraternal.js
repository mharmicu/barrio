    ...
    while (dt <= end) {
        ...
        if (x_fra.indexOf(iso) !== -1) {
            ...
            data2.push({
                ...
                y: isoDayOfWeek(dt),
                d: iso,
                v: y_fra[x_fra.findIndex(indexNum)]
            });
        } else {
            data2.push({
                x: iso,
...
const dataFra = {
    datasets: [{
        label: 'Heat Map - Fraternal St.',
        data: dataFromDBFraternal(),
        backgroundColor(c) {
...
// render init block
const matrixFra = new Chart(
    document.getElementById('matrixChartFraternal'),
    configFra
);