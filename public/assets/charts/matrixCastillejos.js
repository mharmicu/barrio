...
//testing code block 
function dataFromDBCastillejos() {
    const d = new Date();
    ...
    while (dt <= end) {
        ...
        if (x_cas.indexOf(iso) !== -1) {    
            ...
            data2.push({
                ...
                y: isoDayOfWeek(dt),
                d: iso,
                v: y_cas[x_cas.findIndex(indexNum)]
            });
        } else {
            data2.push({
                x: iso,
...
const dataCas = {
    datasets: [{
        label: 'Heat Map - Castillejos St.',
        data: dataFromDBCastillejos(),
        backgroundColor(c) {
...
// render init block
const matrixCas = new Chart(
    document.getElementById('matrixChartCastillejos'),
    configCas
);