    ...
    while (dt <= end) {
        ...
        if (x_pcasal.indexOf(iso) !== -1) {
            ...
            data2.push({
                ...
                y: isoDayOfWeek(dt),
                d: iso,
                v: y_pcasal[x_pcasal.findIndex(indexNum)]
            });
        } else {
            data2.push({
                x: iso,
...
const dataPCasal = {
    datasets: [{
        label: 'Heat Map - Pascual Casal St.',
        data: dataFromDBPCasal(),
        backgroundColor(c) {
...
// render init block
const matrixPCasal = new Chart(
    document.getElementById('matrixChartPCasal'),
    configPCasal
);