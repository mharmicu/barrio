    ...
    while (dt <= end) {
        ...
        if (x_far.indexOf(iso) !== -1) {
            ...
            data2.push({
                ...
                y: isoDayOfWeek(dt),
                d: iso,
                v: y_far[x_far.findIndex(indexNum)]
            });
        } else {
            data2.push({
                x: iso,
...
const dataFar = {
    datasets: [{
        label: 'Heat Map - Farnecio St.',
        data: dataFromDBFarnecio(),
        backgroundColor(c) {
...
// render init block
const matrixFar = new Chart(
    document.getElementById('matrixChartFarnecio'),
    configFar
);