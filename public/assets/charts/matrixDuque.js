    ...
    while (dt <= end) {
        ...
        if (x_duq.indexOf(iso) !== -1) {
            ...
            data2.push({
                ...
                y: isoDayOfWeek(dt),
                d: iso,
                v: y_duq[x_duq.findIndex(indexNum)]
            });
        } else {
            data2.push({
                x: iso,
...
const dataDuq = {
    datasets: [{
        label: 'Heat Map - Duque St.',
        data: dataFromDBDuque(),
        backgroundColor(c) {
...
// render init block
const matrixDuq = new Chart(
    document.getElementById('matrixChartDuque'),
    configDuq
);